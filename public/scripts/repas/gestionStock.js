...
window.onload = function(){
    ...
    var razStock = document.getElementById("RaZStock");
    ...
    razStock.onclick = function(){
        var ret = confirm("Etes-vous sûr de vouloir remettre à zero les quantités des produits non utilisés dans les menus?");
        if(ret){
            var url;
            if(window.location.hostname.indexOf('localhost') === -1)
                url = '/repas/raz';
            else
                url = '/speedself/public/repas/raz';
            ...
            request.done(function(data){
                ...
                location.reload(true);
                flashMessage("Les quantitées ont bien été remise à zero!");
            });
