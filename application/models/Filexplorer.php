...
class Application_Model_Filexplorer
{
    /**
     * @var string directory where the sent attachment files go (with extra slash at the end)
     */
    const ATTACHMENT_DIRECTORY = '../public/exports/sent/';
    
    /**
     * @var array the mime types associated with the good file extension
     */
    private static $mimeAndExt = array(
    ...
    /**
     * Save the files in $files into the webserver
     * They are placed in the tmp directory
     * 
     * @param array $files must receive a var like $_FILES
     * @return multitype:string : array of the attachment paths on the webserver
     */
    public static function saveFilesIntoExplorer($files){
        ...
        for ($i = 0; $i < count($files["files"]['name']); $i ++) {
            ...
            $newPath = self::ATTACHMENT_DIRECTORY.date('Y').'/'.date('M').'/';
            if (!is_dir($newPath)) {
     ...
     * 
     * @param string $extension file extension with a dot (.)
     * @return string the corresponding mime type
     */
    public static function getMimeTypeFromExtension($extension){
        if(array_search($extension, self::$mimeAndExt) === false)
            return "application/octet-stream";
        else 
            return array_search($extension, self::$mimeAndExt);
    }
     ...
     * @return string file extension wi the dot (.)
     */
    public static function getExtensionFromMimeType($mime){
        return (empty(self::$mimeAndExt[$mime])) ?  '.bin' : self::$mimeAndExt[$mime];
    }
    
    /**
     * Check if the post_max_size env variable has been exceeded by the length of the content
     * 
     * @param number $contentLength length of the content (like $_SERVER["CONTENT_LENGTH"])
     * @return boolean true if exceeded, otherwise false
     */
    public static function hasPostMaxSizeBeenExceeded($contentLength){
