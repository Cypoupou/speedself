...
abstract class Zend_Cloud_Infrastructure_Adapter_AbstractAdapter implements Zend_Cloud_Infrastructure_Adapter {
	...
	public function deployInstance($id, $params, $cmd) {
		...
		if (empty ( $params ) || empty ( $params [Zend_Cloud_Infrastructure_Instance::SSH_USERNAME] ) || (empty ( $params [Zend_Cloud_Infrastructure_Instance::SSH_PASSWORD] ) && empty ( $params [Zend_Cloud_Infrastructure_Instance::SSH_KEY] ))) {
			require_once 'Zend/Cloud/Infrastructure/Exception.php';
		...
		if (! ssh2_auth_password ( $conn, $params [Zend_Cloud_Infrastructure_Instance::SSH_USERNAME], $params [Zend_Cloud_Infrastructure_Instance::SSH_PASSWORD] )) {
			require_once 'Zend/Cloud/Infrastructure/Exception.php';
